...
module.exports = function(args) {
  ...
  var readdirpStream = readdirp({root: publicDir, entryType: 'both'});
  
  if (args.extensionless_content_type) {
    var path = require('path');
    readdirpStream = readdirpStream.on('data', function(file) {
      if (!path.extname(file.name)) {
        file.contentType = args.extensionless_content_type;
      }
    });
  }
